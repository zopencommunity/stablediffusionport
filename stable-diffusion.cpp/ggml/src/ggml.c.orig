...
#if defined(__has_include) && __has_include(<alloca.h>)
  #include <alloca.h>
#else
  #include <stdlib.h>          // platforms that lack alloca.h
#if defined(__MVS__) && !defined(GGML_ZOS_POSIX_MEMALIGN)
#define GGML_ZOS_POSIX_MEMALIGN      /* prevent duplicate insertion     */

#include <stdlib.h>
#include <errno.h>                   /* EINVAL / ENOMEM                 */
/* Fallback in the unlikely event errno values are still undefined      */
#ifndef EINVAL
  #define EINVAL  22
#endif
#ifndef ENOMEM
...
static int ggml_posix_memalign(void **memptr,
                               size_t      alignment,
                               size_t      size)
{
    ...
    void *p = aligned_alloc(alignment, size);  /* C11 intrinsic on z/OS */
    if (!p)
        return ENOMEM;
...
#define posix_memalign ggml_posix_memalign
#endif  /* __MVS__ && !GGML_ZOS_POSIX_MEMALIGN */
