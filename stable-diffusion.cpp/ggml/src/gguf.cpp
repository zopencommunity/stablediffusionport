...
struct gguf_reader {
    ...
    bool read(std::string & dst) const {
        ...
        #if defined(__MVS__) || defined(__BIG_ENDIAN__)
...
struct gguf_context * gguf_init_from_file_impl(FILE * file, struct gguf_init_params params) {
    ...
    if (ok && gr.read(ctx->version)) {
        #if defined(__MVS__) || defined(__BIG_ENDIAN__)
            ctx->version = __builtin_bswap32(ctx->version);
        #endif
        if (ctx->version == 1) {
...
if (ok && gr.read(n_tensors)) {
    #if defined(__MVS__) || defined(__BIG_ENDIAN__)
        n_tensors = __builtin_bswap64(n_tensors);
    #endif
    ...
    // KV pairs
    {
        for (int64_t i = 0; ok && i < n_kv; ++i) {
            ...
            ok = ok && gr.read(type);
            #if defined(__MVS__) || defined(__BIG_ENDIAN__)
                type = (gguf_type)__builtin_bswap32((uint32_t)type);
            #endif
            ...
            // tensor shape
{
    ...
    #if defined(__MVS__) || defined(__BIG_ENDIAN__)
        n_dims = __builtin_bswap32(n_dims);
    #endif
    if (n_dims > GGML_MAX_DIMS) {
            ...
            if (j < n_dims) {
                ...
                #if defined(__MVS__) || defined(__BIG_ENDIAN__)
                    info.t.ne[j] = __builtin_bswap64(info.t.ne[j]);
                #endif
                }
        ...
        // tensor type
        {
            ...
            #if defined(__MVS__) || defined(__BIG_ENDIAN__)
                info.t.type = (ggml_type)__builtin_bswap32((uint32_t)info.t.type);
            #endif
    ...
    #if defined(__MVS__) || defined(__BIG_ENDIAN__)
        info.offset = __builtin_bswap64(info.offset);
    #endif
